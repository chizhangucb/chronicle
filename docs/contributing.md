...
- **Single source of truth for shared vocabulary.** Chat-type labels live only in
  `src/kinds.ts`; the friendly label for a raw tool name lives only in `src/toolLabels.ts`;
  per-model context windows and prices live only in `src/models.ts`. Add new wording or
  numbers there, never inline.

...
- `≈` is the marker for "not a billed total", not for calibration alone. Explore's `hour`
  and `subagent` dimensions are neither exact nor calibrated: they sum the per-message token
  columns, which hold only ~0.73 of billed usage and have no hourly or per-agent-type
  counterpart in `sessions.usage` to be scaled against. They carry `≈` too (#203), with their
  own definition (`explore.approximate-tokens`) behind the ⓘ saying which approximation it
  is. `src/explore/tokenColumns.ts` is the one place a dimension is classified exact /
  calibrated / partial, so the card and the Detail table cannot disagree again.

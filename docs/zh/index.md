...
Chronicle 的设计理念是 **回放 · 度量 · 安全 (Replay · Measure · Secure)**：
...
- **度量 (Measure)** —— 带有本地计算的成本和存储时长指标的
  [会话洞察](guide/session-insights.md)、子代理（侧链）归因、由 FTS5 支撑的
  [全文搜索](guide/search-and-filtering.md)、保持一切新鲜的后台
  [自动同步](guide/auto-sync.md)，以及面向外部消费者的带版本
  [SQL 契约视图](architecture/metrics-and-contract.md)。
- **安全 (Secure)** —— 一键 [安全检查与脱敏](guide/security-and-sharing.md)，以及本地托管的
  脱敏分享链接。所有解析与存储都留在设备上（参见
...
| [搜索与筛选](guide/search-and-filtering.md) | 类型筛选标签、`⌘F` 搜索，以及由 FTS5 支撑的 `⌘K` 命令面板 |
...
| [自动同步](guide/auto-sync.md) | 后台增量导入、「进行中」指示器，以及 `chronicle://` 深度链接 |
| [安全与分享](guide/security-and-sharing.md) | 安全检查、自定义规则，以及分享链接 |
...
| [指标与契约视图](architecture/metrics-and-contract.md) | 侧链与逐消息 token 列、存储的时长、`contract_*` 视图，以及 FTS5 |
| [安全、实时与回放](architecture/security-live-replay.md) | 脱敏引擎、SSE 监听器、回放引擎，以及因果分析 |
| [API 参考](architecture/api-reference.md) | 每一条 REST 路由、SSE 流，以及 `/share` |

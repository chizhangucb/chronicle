...
transcript, so the migration rebuilt the numbers structurally from the surviving
...
| Sessions | `GET /sessions/:id/messages`, `PATCH /sessions/:id`, `DELETE /sessions/:id`, `DELETE /sessions/:id/source-file`, `POST /sessions/undo-delete`, `GET /sessions/minor`, `POST /sessions/:id/promote`, `GET /sessions/:id/live` (SSE), `GET /sessions/:id/security-check`, `GET /sessions/:id/export-redacted` |

...
Chronicle is local-first: it reads your session history and local git repos on your machine, your session data never leaves it, and it has no server of its own. The one outbound call is the Claude plan-window read: your own token, to that token's own issuer, for your own plan windows, on by default and off in Settings. That also means Chronicle only works while it's running. If you want your Chronicle available at a stable URL all day (bookmarkable, survives reboots), run it as a login service instead of starting it by hand.

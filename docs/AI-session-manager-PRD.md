...
**Document status:** v1.0 — implemented through Phase 5 (see §9 Decision log); v0.1.0 released 2026-07-06, v0.1.6 (signed + notarized, auto-update) 2026-07-09, **v0.1.7 (session UX polish) 2026-07-09**
**Author:** Chi Zhang
**Date:** 2026-07-03
...
**Implementation status:** Phases 1–5 complete except **remote SSH (§5.9)** — all 6 tool importers, time travel, Replay, Refine, Overview + mode rail, filtering, analytics, live streaming (JSONL + SQLite polling), security (redaction, pre-tool-use hook, share links), MCP Hub, Skills Hub, i18n (EN/zh-CN/ja), Electron desktop shell. **FR-IMP-1** is mostly shipped: macOS `.dmg` (arm64 + x64) + Homebrew cask, and — as of **v0.1.6 (2026-07-09)** — Apple **Developer ID code signing + notarization** and **one-click auto-update (NFR-7)** via `electron-updater` with an in-app "Relaunch to update" toast; Windows/Linux installers remain pending. Context-window accounting was added beyond the PRD: real per-session token usage parsed from Claude Code usage records, with a usage bar against each model's context window. Feedback delivery moved to the branded `getchronicle.dev` domain (hosted Vercel relay → Resend → Porkbun forwarding), keeping the maintainer's personal address out of the repo and bundle. **v0.1.7 (2026-07-09)** shipped a session-UX-polish batch via auto-update: an Active Duration stat (real working time, idle gaps excluded), Refine delete-by-type, consolidated role-accurate chat labels across Playback/Refine, a `⇧⌘U` session-sync shortcut, an optional feedback sender email set as `Reply-To`, and a fix so switching language keeps your current page. **v0.1.9 (2026-07-12)** shipped via auto-update: home-page multi-select project delete (inline confirm), an always-on top bar (Search + Import in every view), a corrected Active Duration (counts all agent thinking/tool time, excluding only the pause before each user prompt — replacing the old 5-minute idle cutoff), cache-write cost split into 5m/1h TTL tiers in the Cost & Usage panel, and downward-opening ⓘ tooltips so long explainers stay readable. **v0.1.10 (2026-07-12)** refined that metric: "Active Duration" was renamed **Agent Active** and now excludes only genuine human prompts — background-task completions, in-app element selections, and interrupt markers all carry a `user` role, so the prior logic wrongly charged background-build waits to the user's idle time.

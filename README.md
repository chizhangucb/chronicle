...
own* Claude plan quota by sending Claude Code's own OAuth token to the token's own
issuer (the same request Claude Code makes; Codex quota is read locally). Source logs
and project repos are never written to.

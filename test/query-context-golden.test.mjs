// Query-context golden (ticket #304): every engine's JSON, on one
// deterministic corpus, pinned against the pre-slice commit (`main` at
// 15d1589). The consolidation onto one query context is behaviour-preserving
// by intent, so equality with the fixture IS the assertion — every engine
// except activity's baseline, which this slice fixes on purpose (excluded
// here, pinned by test/activity-baseline-local-days.test.mjs instead).
//
...
// matrix, which is what this slice reparameterized. Both are kept.
...
const GOLDEN = JSON.parse(fs.readFileSync(new URL('./fixtures/query-context-golden.json', import.meta.url), 'utf8'));
...
before(async () => {
  ...
  const { computeExplore, toWire } = await import('../server/explore.ts');
  ...
  for (const c of exploreCases) {
    ...
    // toWire, so the pinned cells carry Explore's frozen wire names
    // (cw5m/cw1h) — the same JSON /api/explore serves (#301, server/explore.ts).
    out[`explore:${key}`] = toWire(computeExplore({ scope: ALL, range: rangeOf(days, now), ...rest }));
    out[`explore-proj:${key}`] = toWire(computeExplore({ scope: { type: 'project', id: alpha.id }, range: rangeOf(days, now), ...rest }));
  }
...
for (const key of Object.keys(GOLDEN)) {
  test(`query-context golden: ${key} is unchanged by the query-context consolidation`, () => {

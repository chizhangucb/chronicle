...
const zh = {
  ...
  'Hit rate': '命中率',
  'Msgs': '消息数',
  'Token usage by model': '按模型统计的词元用量',
  'Session total': '会话总计',
  'Cache read ÷ (cache read + input): the share of prompt-side tokens served from cache instead of re-sent at full input price. Higher = cheaper turns.':
    '缓存读取 ÷ (缓存读取 + 输入)：提示词侧词元中由缓存提供、而非按全价输入重新发送的占比。数值越高，单轮对话越省钱。',
...
const ja = {
  ...
  'Cache read ÷ (cache read + input): the share of prompt-side tokens served from cache instead of re-sent at full input price. Higher = cheaper turns.':
    'キャッシュ読み取り ÷ (キャッシュ読み取り + 入力)：プロンプト側トークンのうち、全額の入力コストで再送されずキャッシュから提供された割合。値が高いほど1ターンあたりのコストが安くなります。',

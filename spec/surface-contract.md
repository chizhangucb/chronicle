...
- **Explore's total bar and its stacked time chart report the same number.** A session whose
  activity spans the range edge contributes its in-range share to each rollup bucket, exactly as it
  already did to the ranked rows, so summing a chart's buckets reproduces that chart's own total at
  every rollup granularity. Spec #294 changed these numbers on purpose (the rollup used to place a
  spanning session's whole billed usage on the bucket it started in, so the chart read higher than
  its total); Chi's confirmation on #271 and #294 is the sign-off. Guards:
...
- **Explore attributes an error per message only where it must.** The `project`, `source` and
  ...
  same whole-session windowing tradeoff Insights' error rate does: a session overlapping the range
  edge reports its full error count. `tool`, `skill`, `model`, `subagent`, `mcp`, `provider` and

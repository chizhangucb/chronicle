...
| any other path | **Not found.** The fallback surface: a path matching NO route above renders it, in the shared `page center empty-state` grammar inside the normal app frame (sidebar + topbar). It states that the page does not exist and links back to Insights, and says NOTHING else: it never explains why, never names a surface Chronicle dropped and never uses the retired vocabulary, because a mistyped path and a bookmark from an older release are indistinguishable from here. Explaining a dropped surface is the Reference page's `Retired` group's job, and it is linked from the sidebar. The route table both halves read is `src/routes.ts`: every match above comes from it, asked through the router's own matcher, so a page added there is never shadowed by this one. | `src/NotFoundPage.tsx` |
...
| No scheduled-job template is tracked, so none can ship in the published tarball | `test/repo-shape.test.mjs` — "the published package ships no job template" |
...
| An unrouted path renders the not-found surface and a routed one does not; the surface's copy explains nothing and names no dropped surface | `test/not-found-route.test.mjs` |

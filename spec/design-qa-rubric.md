...
- **Mono glyph vocabulary, no colored emoji, in chrome or page content.** The canonical set
  ...
  `↶`/`↷`=undo/redo (`src/RefineMode.tsx`) `↧`=export to a file (`src/RefineMode.tsx`; distinct
  from `⇩`=imported, `src/ImportWizard.tsx`). Message kinds (`src/kinds.ts` `KIND_ICON`, the marker
  on every Playback row): `⊙`=user `✳`=assistant `⋯`=thinking `⇥`=tool call (it hands off; `↩`=tool
  result is what comes back) `＋`=inserted. This list is canonical but not exhaustive — any
  other MONO glyph used consistently for one meaning is legitimate; a COLORED emoji is not, full
  stop, and there is no carve-out. **Pinned by `test/no-colored-emoji.test.mjs`**, which sweeps
  `src/`, `server/`, `shared/`, `bin/` and this contract for any code point whose DEFAULT
  presentation is the color emoji glyph. Published prose (`docs/`, `website/`) is out of that
  scope: it is not app chrome, and its support matrix uses status emoji as table data. `test/kind-icons.test.mjs`
  pins the kind glyphs themselves, in the map and in the row that draws them.
